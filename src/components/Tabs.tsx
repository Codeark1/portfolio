  ...
  return (
    <header className="flex items-center justify-between px-4 py-2  text-white relative font-sen bg-darkBackground">
      ...
      <nav className="hidden lg:flex items-center space-x-6 sticky ">
        ...
        <TabLink href="/project" icon={<CiFolderOn size={24} />} label="Project" />
      </nav>

      {/* Blog Button */}
      <div className="hidden lg:block  ">
        <Button className="!bg-darkBackground text-white font-sen">Blog</Button>
      </div>
      ...
      {isMenuOpen && (
        <div className="absolute top-full left-0 w-full bg-darkBackground text-white z-50 flex flex-col items-center space-y-4 py-6">
          ...
          <TabLinkMobile href="/about" icon={<IoInformation size={24} />} label="About" />
          <TabLinkMobile href="/tools" icon={<FiTool size={24} />} label="Tools" />
          <TabLinkMobile href="/project" icon={<CiFolderOn size={24} />} label="Project" />
...
const TabLink: React.FC<TabLinkProps> = ({ href, icon, label }) => (
  <div className="group flex flex-col items-center cursor-pointer">
    <Link href={href} className="flex flex-col items-center">
      <div className="text-white group-hover:text-blue-500 transition-all">{icon}</div>
...
const TabLinkMobile: React.FC<TabLinkProps> = ({ href, icon, label }) => (
  <Link href={href} className="flex items-center space-x-3 px-4 py-2 w-full hover:bg-gray-700 rounded transition-all">
    <div>{icon}</div>
    <span>{label}</span>
  </Link>
);
